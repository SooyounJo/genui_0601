(function () {
  ...
  var FRAG_SRC = [
    ...
    '    float xDist = max((uv.x - u_origin.x + 0.015) * u_aspect, 0.0);',
    '    float ripple = sin(time * 1.28 + uv.y * 9.0 + fbm(uv * 3.0 + time * 0.052) * 3.0) * 0.046;',
    '    float wobble = (fbm(uv * 2.6 + time * 0.028) - 0.5) * 0.058 * max(s, 0.1);',
    '    float front = s * 1.24 + ripple + wobble;',
    '    float soft = 0.26 + s * 0.14;',
    '    float reveal = 1.0 - smoothstep(front - soft * 0.62, front + soft * 0.92, xDist);',
    '    float wide = 1.0 - smoothstep(front - soft * 1.18, front + soft * 1.42, xDist);',
    '    reveal = max(reveal, wide * 0.34) * onset;',
    '    float orbPulse = exp(-dot(rel, rel) * 2.0) * (1.0 - smoothstep(0.14, 0.66, s)) * onset;',
    '    return clamp(max(reveal, orbPulse), 0.0, 1.0);',
    '  }',
    '  float onset = smoothstep(0.0, 0.09, s);',
    ...
    '  if (u_variant > 0.5) {',
    '    float warpAmt = 0.020 + audio * 0.007;',
    '    vec2 warp = vec2(',
    '      fbm(uv * 1.6 + time * 0.038) - 0.5,',
    '      fbm(uv * 1.6 + vec2(9.0, 6.5) + time * 0.034) - 0.5',
    '    ) * warpAmt;',
    '    vec2 u = uv + warp;',
    '    float x = u.x;',
    '    float y = u.y;',
    '    vec3 deepOrange = vec3(1.0, 0.498, 0.141);',
    '    vec3 warmMid = vec3(1.0, 0.667, 0.298);',
    '    vec3 cream = vec3(1.0, 0.929, 0.718);',
    '    vec3 whitePeak = vec3(1.0, 0.992, 0.964);',
    '    vec3 col = mix(deepOrange, warmMid, smoothstep(0.0, 0.32, x));',
    '    col = mix(col, cream, smoothstep(0.26, 0.68, x));',
    '    col = mix(col, whitePeak, smoothstep(0.54, 1.0, x) * 0.92);',
    '    float breath = 0.5 + 0.5 * sin(time * 1.42);',
    '    float breathSoft = breath * breath * (3.0 - 2.0 * breath);',
    '    float ripple = sin(x * 8.8 - time * 1.42 + fbm(u * 2.7 + time * 0.056) * 3.4) * 0.5 + 0.5;',
    '    ripple *= sin(y * 7.2 + time * 1.12 + fbm(u * 2.1 + time * 0.042) * 2.4) * 0.5 + 0.5;',
    '    float whiteLobe = smoothstep(0.40, 0.90, x) * ripple;',
    '    col = mix(col, whitePeak, whiteLobe * mix(0.42, 0.82, 1.0 - breathSoft));',
    '    float shimmer = (sin(x * 11.5 - time * 1.72 + fbm(u * 2.0 + time * 0.050) * 3.0) * 0.5 + 0.5)',
    '      * (sin(y * 5.8 + time * 0.96) * 0.5 + 0.5);',
    '    col += whitePeak * shimmer * 0.13 * smoothstep(0.34, 0.98, x);',
    '    float distEdge = min(min(x, 1.0 - x), min(y, 1.0 - y));',
    '    float rim = smoothstep(0.18, 0.0, distEdge);',
    '    col = mix(col, vec3(1.0, 0.86, 0.58), rim * rim * 0.22);',
    '    return col * 1.04;',
    '  }',
